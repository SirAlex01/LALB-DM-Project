...
def collect_lin_b_words(linear_b_file):
    ...
    with open(linear_b_file, 'r', encoding='utf-8') as f:
        for i, r in enumerate(f):
            ...
            words[word] = {"completeness level": valid, "cognates":[]}
    return words
...
def make_prompt(word, info_dict, api_key):
    ...
    # Core rules
    rules = [
    	"OUTPUT: Maximum 3 cognates separated by commas.",
    ...
    ET.SubElement(word_elem, "completeness_level").text = str(info_dict["completeness level"])
    ...
    output_rules = [
        ...
        "FORMAT: Output up to 3 cognates."
    ]
...
if not file_exists:
    with open(out, "w", encoding="utf-8") as f:
        f.write("converted_linear_b\tgreek\tgemini\tcompleteness_level\tlikelihood\tsequence_id\n")
        f.flush()
    start_index = 0
else:
...
with open(seq_lists_file, "r", encoding="utf-8") as lin_b_words_file:
    for _ in range(2): # Skip the header
...
for i, word in tqdm(enumerate(sorted(matching.keys())[start_index:], start=start_index)):
    ...
    with open(out, "a", encoding="utf-8") as f_append:
        f_append.write(f"{word}\t{to_insert}\t{gemini_cogs}\t{matching[word]['completeness level']}\t{gemini_likelihoods}\t{seq_list}\n")

    with open(f"gemini_output{SUFFIX}.jsonl", "a", encoding="utf-8") as f_jsonl:
        json_line = json.dumps({word: {"completeness level": matching[word]['completeness level'], "output": gemini_cognates}}, ensure_ascii=False)
